...
void Station::manage(vector<Train> &vect, const int time)
{
    ...
    for (int i = 0; i < vect.size(); i++)
    {
        if (vect[i].current_pos < distance - 5) //se è nella zona senza limiti di velocità
        {
            ...
            else if (vect[i].current_pos >= distance - 5 && vect[i].current_speed > 80.00 / 60.00 && !vect[i].is_parked) //quando arriva a 5km(e non è già nel parcheggio)
            {
            ...
            else if (vect[i].current_pos >= distance && vect[i].current_pos < distance + (80.00 / 60.00) - 0.1) //- 1 + 2.00 / 3 + (80.00 / 60.00) + 1.00 / 3) //se il treno è  nella stazione (0.1 approssimazione della distanza)
            {
                if (vect[i].wait == 0)
                {
    ...
    void Station::start(vector<Train> & vect, const int time)
    {
        ...
        for (int i = 0; i < vect.size(); i++)
        {
            if (i != 0 && (vect[i].start_time - vect[i - 1].start_time) < 6) //se due treni hanno orari di partenza troppo vicini l'orario di partenza del secondo treno viene aumentato di 6(tempo necessario per dare modo al primo treno di prendere 10km di vantaggio)
            {
